...
public class DBParams {
     ...
     */
    private static final int[] SQLTYPES = { Types.INTEGER, Types.BIGINT, Types.VARCHAR, Types.DATE, Types.TIMESTAMP,
            Types.DOUBLE, Types.TIME };

    /**
     * Java数据基本类型，SQLTYPES与CLASSTYPES类型是一一对应的
     */
    private static final Class<?>[] CLASSTYPES = { Integer.class, Long.class, String.class, Date.class, Timestamp.class,
            Double.class, Time.class };

    // 访问数据库，更新、删除、修改、查询、添加的sql语句预先定义的语句参数
    private List<DBParamsEntity> paramList = new ArrayList<DBParamsEntity>();
    ...
    public DBParams copy() {
        ...
        for (DBParamsEntity entity : paramList) {
    ...
    /***
     * 为sql语句设置参数如： select * from t_logistics_road t where t.roadid=?
     * 
     * @param o
     *            添加的参数对象，此对象可以是任意的数据基本类型和类
     */
     ...
     * 
     * @param type
     *            Java 编程语言中所有类型的公共高级接口。 它们包括原始类型、参数化类型、数组类型、类型变量和基本类型
     * @return
     */
     ...
     *            添加参数设置对象
     * @param type
     *            jdbc数据库数据类型，如Types.INTEGER, Types.BIGINT, Types.VARCHAR等
     */
    private void addParam(Object o, int type) {
        DBParamsEntity entity = new DBParamsEntity(o, type);
