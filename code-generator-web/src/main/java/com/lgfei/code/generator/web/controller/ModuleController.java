...
public class ModuleController extends BaseController<IModuleService, Module, Long> {
    ...
    @Override
    protected Module fillNo(Module entity) {
        if (null == entity || !StringUtils.isEmpty(entity.getModuleNo())) {
            return entity;
        }
        String nextNo = sysCodeRuleService.getNextNo("module", "module_no");
        if (!StringUtils.isEmpty(nextNo)) {
            entity.setModuleNo(nextNo);
        }
        return entity;
    }
